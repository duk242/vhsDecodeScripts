...
# This is Harry's Align script - modified to output to the same directory the input file is in - will also use VhsDecodeAutoAudioAlign.exe from the same folder as the script.
...
AAA_EXE="$SCRIPT_DIR/VhsDecodeAutoAudioAlign.exe"
...
if [[ -f "$AAA_EXE" ]]; then
  ...
  echo "Error: aaa.exe not found in expected location"
  echo "  - $AAA_EXE"
  exit 1
fi
